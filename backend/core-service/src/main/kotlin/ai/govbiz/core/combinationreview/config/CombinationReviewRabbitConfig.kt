...
class CombinationReviewRabbitConfig {
    @Bean
    fun combinationReviewQueues(): Declarables {
    ...
    @Bean
    fun combinationReviewListenerContainerFactory(connectionFactory: ConnectionFactory,
    ...
    @Bean
    fun combinationReviewOutboxTaskScheduler() = ThreadPoolTaskScheduler().apply {
        poolSize = 1
        setThreadNamePrefix("combination-review-outbox-")
    }

    companion object {
        const val EXCHANGE = "govbiz.combination-review.generation.v1"
        const val QUEUE = "govbiz.combination-review.generation.v1"
        const val DEAD_EXCHANGE = "govbiz.combination-review.generation.dead.v1"
        const val DEAD_QUEUE = "govbiz.combination-review.generation.dead.v1"
    }
}
