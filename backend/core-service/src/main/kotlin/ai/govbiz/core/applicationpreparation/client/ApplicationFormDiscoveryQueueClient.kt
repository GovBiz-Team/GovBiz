package ai.govbiz.core.applicationpreparation.client
...
class ApplicationFormDiscoveryQueueClient(private val rabbit: RabbitTemplate) {
    fun publish(jobId: Long) {
        ...
        rabbit.send(ApplicationFormDiscoveryRabbitConfig.EXCHANGE, ApplicationFormDiscoveryRabbitConfig.QUEUE, message, correlation)
        val confirm = correlation.future.get(3, TimeUnit.SECONDS)
        check(confirm.isAck && correlation.returned == null) { "Application form discovery queue did not accept the job" }
    }
}
