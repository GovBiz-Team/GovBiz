...
class DailyReportRabbitConfig {
    @Bean
    fun dailyReportQueues(): Declarables {
    ...
    @Bean
    fun dailyReportListenerContainerFactory(connectionFactory: ConnectionFactory) = SimpleRabbitListenerContainerFactory().apply {
    ...
    @Bean
    fun dailyReportOutboxTaskScheduler() = ThreadPoolTaskScheduler().apply {
        poolSize = 1
        setThreadNamePrefix("daily-report-outbox-")
    }

    companion object {
        const val EXCHANGE = "govbiz.daily-report.generation.v1"
        const val QUEUE = "govbiz.daily-report.generation.v1"
        const val DEAD_EXCHANGE = "govbiz.daily-report.generation.dead.v1"
        const val DEAD_QUEUE = "govbiz.daily-report.generation.dead.v1"
    }
}
